...
const TrendChart = ({ data, toolType }) => {
    if (!data || data.length === 0) return null;

    // Process data for chart
    // Smart Sampling:
    // 1. Always show the FIRST point (Baseline).
    // 2. Always show the LAST point (Current).
    // 3. If > 50 points, sample evenly in between.

    let processedData = [...data].reverse(); // Chronological: Oldest -> Newest

    if (processedData.length > 50) {
        const baseline = processedData[0];
        const current = processedData[processedData.length - 1];
        const middle = processedData.slice(1, processedData.length - 1);

        // Sample middle points
        const step = Math.ceil(middle.length / 48);
        const sampledMiddle = middle.filter((_, index) => index % step === 0);

        processedData = [baseline, ...sampledMiddle, current];
    }

    const chartData = processedData.map(item => ({
        date: new Date(item.created_at).toLocaleDateString(),
        fullDate: new Date(item.created_at).toLocaleString(),
        value: parseFloat(item.data.resistance || item.data.value || 0),
        unit: item.data.unit || '',
        rating: item.data.voltageRating || ''
    }));

    // Determine thresholds based on tool type (currently optimized for Megohmmeter)
    let dangerLine = 5;
    let warningLine = 50;

    // Check if we are dealing with High Voltage to adjust lines (heuristic based on last item)
