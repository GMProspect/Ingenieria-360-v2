...
const Megohmetro = () => {
    ...
    const evaluateInsulation = (res, unit, rating) => {
        ...
        // Define Thresholds based on Voltage Rating
        let minAcceptable = 5; // Default safety baseline
        let warningThreshold = 100;

        const isLowVoltage = ['110', '220', '480'].includes(rating);
        ...
        if (r < minAcceptable) {
            return {
                ...
                message: `Resistencia (${r.toFixed(2)} MΩ) por debajo del límite mínimo de ${minAcceptable} MΩ.`,
                action: 'NO ENERGIZAR. Indica falla a tierra franca o humedad severa en el bobinado. Requiere secado urgente o rebobinado.',
                color: 'text-red-500',
                bgColor: 'bg-red-500/10',
                borderColor: 'border-red-500/50',
                glowColor: 'red'
            };
        } else if (r >= minAcceptable && r < warningThreshold) {
            return {
                status: 'warning',
                title: isLowVoltage ? 'PRECAUCIÓN (Posible Humedad)' : 'ALERTA (Bajo Aislamiento)',
                message: `Valor (${r.toFixed(2)} MΩ) aceptable para operar, pero indica contaminación.`,
                action: `Rango típico de ${isLowVoltage ? 'humedad o suciedad superficial' : 'envejecimiento del aislamiento'}. Programar limpieza y secado en próxima parada.`,
            ...
            return {
                ...
                message: `Valor (${r.toFixed(2)} MΩ) excelente. Sistema seco y limpio.`,
                action: 'Condiciones ideales de operación. Registrar valor en historial para análisis de tendencia.',
